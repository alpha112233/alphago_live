...
class IIFLXTSExchangeMapper:
    ...
    @staticmethod
    def get_openalgo_exchange(iiflxts_code):
        """
        Convert IIFL XTS exchange code to OpenAlgo exchange code

        Args:
            iiflxts_code (int): IIFL exchange code

        Returns:
            str: OpenAlgo exchange code
        """
        return IIFLXTSExchangeMapper.REVERSE_EXCHANGE_TYPES.get(
            iiflxts_code, "NSE"
        )  # Default to NSE if not found


class IIFLXTSCapabilityRegistry:
    """
