...
from broker.iiflxts.streaming.iiflxts_websocket import IIFLXTSWebSocketClient
...
from .iiflxts_mapping import IIFLXTSCapabilityRegistry, IIFLXTSExchangeMapper


class IIFLXTSWebSocketAdapter(BaseBrokerWebSocketAdapter):
    """IIFL XTS specific implementation of the WebSocket adapter"""

    def __init__(self):
        super().__init__()
        self.logger = logging.getLogger("iifl_xts_websocket")
        self.ws_client = None
        self.user_id = None
        self.broker_name = "iiflxts"
        ...
        self.logger.info(f"IIFL XTS adapter initialized with ZMQ port: {self.zmq_port}")
    ...
    def initialize(
        ...
        Args:
            broker_name: Name of the broker (always 'iiflxts' in this case)
        ...
        # Create IIFL XTS WebSocket client with API credentials
        self.ws_client = IIFLXTSWebSocketClient(
            api_key=api_key,
    ...
    def disconnect(self) -> None:
        ...
        self.logger.info("*** DISCONNECT CALLED - Starting IIFL XTS disconnect process ***")
    ...
    def subscribe(
        ...
        if mode == 3:  # Depth mode
            if not IIFLXTSCapabilityRegistry.is_depth_level_supported(exchange, depth_level):
                # If requested depth is not supported, use the highest available
                actual_depth = IIFLXTSCapabilityRegistry.get_fallback_depth_level(
        ...
        exchange_type = IIFLXTSExchangeMapper.get_exchange_type(brexchange)
    ...
    def _get_exchange_segment(self, exchange: str) -> str:
        ...
        """
        return IIFLXTSExchangeMapper.get_exchange_type(exchange)
    ...
    def unsubscribe(self, symbol: str, exchange: str, mode: int = 2) -> dict[str, Any]:
        ...
        instruments = [
            {
                "exchangeSegment": IIFLXTSExchangeMapper.get_exchange_type(brexchange),
    ...
    def _process_json_data(self, data: dict):
        ...
        try:
            ...
            # Based on IIFL XTS API documentation:
